...
const factorFor = (
  ...
  return (
    toNumber(
      units.find((unit) => unit.product_id === productId && unit.uom_code === uom)
        ?.conversion_to_base,
    ) || 1
  );
};
...
const statusClass = (status: RowStatus) => {
  ...
  if (status === "Damaged") return "border-amber-200 bg-amber-50 text-amber-700";
...
export default function GoodsReceivingForm({ onBack, onCompleted }: Props) {
  ...
  const { data: currentEmployee } = useQuery({
    ...
    queryFn: async (): Promise<EmployeeLite | null> => {
      ...
      const { data, error } = await (supabase as any)
        .from("employees")
        .select("employee_id, employee_code, display_name, first_name, last_name, auth_user_id")
  ...
  const { data: purchaseOrders = [] } = useQuery({
    ...
    queryFn: async (): Promise<PurchaseOrder[]> => {
      const { data, error } = await (supabase as any)
        ...
        .select(`
          purchase_order_id, purchase_order_no, supplier_id, project_id, site_id,
          order_status, expected_delivery_date,
          suppliers (supplier_code, supplier_name),
          projects (project_no, project_name),
          project_sites (site_code, site_name)
        `)
        .eq("is_deleted", false)
        .in("order_status", ["Confirmed", "Partially Delivered"])
        .not("site_id", "is", null)
  ...
  const selectedPO =
    purchaseOrders.find((po) => po.purchase_order_id === purchaseOrderId) ?? null;
  ...
  const { data: poLines = [] } = useQuery({
    ...
    queryFn: async (): Promise<PurchaseOrderLine[]> => {
      const { data, error } = await (supabase as any)
        ...
        .select(`
          purchase_order_line_id, purchase_order_id, product_id, line_no, description,
          quantity, unit_of_measure, purchase_uom_code, base_uom_code,
  ...
  const productIds = useMemo(
    () => Array.from(new Set(poLines.map((line) => line.product_id).filter(Boolean))),
    [poLines],
  );
  ...
  const { data: stockLocations = [] } = useQuery({
    queryKey: ["goods-receiving-form-stock-locations", selectedPO?.site_id],
    enabled: Boolean(selectedPO?.site_id),
    queryFn: async (): Promise<StockLocation[]> => {
      const { data, error } = await (supabase as any)
        .from("stock_locations")
        .select("stock_location_id, location_code, location_name, site_id")
        .eq("site_id", selectedPO!.site_id)
        .eq("is_active", true)
        .eq("is_deleted", false)
  ...
  const confirmPoMatch = (checked: boolean) => {
    ...
    if (checked && poLines.length > 0 && lineStates.length === 0) initialiseLines(poLines);
  };
  ...
  const lineMath = (line: PurchaseOrderLine, state: LineState) => {
    ...
    const damagedBase =
      ...
      factorFor(line.product_id, state.damagedUom, productUnits, docUom, docFactor);
    const rejectedBase =
      toNumber(state.rejectedQty) *
      factorFor(line.product_id, state.rejectedUom, productUnits, docUom, docFactor);
    const shortBase =
      toNumber(state.shortQty) *
      factorFor(line.product_id, state.shortUom, productUnits, docUom, docFactor);

    const accountedBase = acceptedBase + damagedBase + rejectedBase + shortBase;
    const remainingBase = billBase - accountedBase;
    const remainingDocQty = docFactor > 0 ? remainingBase / docFactor : remainingBase;
    const acceptedDocQty = docFactor > 0 ? acceptedBase / docFactor : acceptedBase;
    const epsilon = 0.000001;
    const balanced = Math.abs(remainingBase) <= epsilon;
    const valid = remainingBase >= -epsilon;

    const positiveBuckets = [acceptedBase, damagedBase, rejectedBase, shortBase].filter(
      (value) => value > epsilon,
    ).length;
  ...
  const summary = useMemo(() => {
    return lineStates.reduce(
      (acc, state) => {
        ...
        acc.accepted += math.acceptedBase;
        acc.damaged += math.damagedBase;
        acc.rejected += math.rejectedBase;
        acc.short += math.shortBase;
        return acc;
      },
  ...
  const uploadFile = async (file: File, photoType: string, deliveryId: string) => {
    if (!file.type.match(/^image\/(jpeg|png|webp)$/)) {
      throw new Error("Use JPEG, PNG or WebP images only.");
    }
    if (file.size > 10 * 1024 * 1024) throw new Error("Image must not exceed 10 MB.");
    ...
    const { error } = await (supabase as any).from("supplier_delivery_photos").insert({
  ...
  const validateEntry = () => {
    if (!deliveryBillNo.trim()) throw new Error("Delivery Bill No. is required.");
    if (!billPhoto) throw new Error("Delivery Bill photo is required.");
    if (!purchaseOrderId || !poMatched) throw new Error("Confirm the Purchase Order match first.");
    if (!stockLocationId) throw new Error("Stock Location is required.");
    if (!currentEmployee) throw new Error("Your login is not linked to an active Employee record.");

    const selected = lineStates.filter((line) => line.selected);
    if (!selected.length) throw new Error("Select at least one item to receive.");
    ...
    selected.forEach((state) => {
      ...
      if (toNumber(state.billQty) <= 0) {
        throw new Error(`Delivery Bill quantity must be greater than zero on line ${line.line_no}.`);
      }
      if (toNumber(state.receivedQty) < 0) {
        throw new Error(`Accepted / Good quantity cannot be negative on line ${line.line_no}.`);
      }
      const math = lineMath(line, state);
      if (!math.valid) {
        throw new Error(`Allocated quantity exceeds the Delivery Bill quantity on line ${line.line_no}.`);
      }
      if (!math.balanced) {
        throw new Error(
      ...
      if (math.damagedBase > 0) {
        if (!state.damageDescription.trim()) {
          throw new Error(`Damage description is required on line ${line.line_no}.`);
        }
      ...
      if (math.rejectedBase > 0) {
        if (!state.rejectionReason.trim()) {
          throw new Error(`Rejection reason is required on line ${line.line_no}.`);
        }
        if (!state.rejectedPhotos.length) {
          throw new Error(`Rejected goods photo is required on line ${line.line_no}.`);
        }
      }
      if (math.shortBase > 0 && !state.shortReason.trim()) {
        throw new Error(`Short / Missing reason is required on line ${line.line_no}.`);
      }
    });

    if (!deliveredGoodsPhotos.length) throw new Error("Delivered Goods photo is required.");
  };
  ...
  const confirmReceiving = useMutation({
    mutationFn: async () => {
      ...
      const { data: createData, error: createError } = await (supabase as any).rpc(
      ...
      if (!deliveryId) throw new Error("Created Supplier Delivery ID was not returned.");
      ...
      const deliveryItems = (deliveryData?.supplier_delivery_items ?? []) as Array<{
      ...
      const receiptItems = selectedStates.map((state) => {
        ...
        if (!deliveryItem) {
          throw new Error(`Created delivery line ${poLine.line_no} could not be matched.`);
        }
        ...
        return {
          ...
          accepted_components:
            math.acceptedBase > 0
              ? [{ quantity: toNumber(state.receivedQty), uom_code: state.receivedUom }]
              : [],
          damaged_components:
            math.damagedBase > 0
              ? [{ quantity: toNumber(state.damagedQty), uom_code: state.damagedUom }]
              : [],
          rejected_components:
            math.rejectedBase > 0
              ? [{ quantity: toNumber(state.rejectedQty), uom_code: state.rejectedUom }]
              : [],
          short_components:
            math.shortBase > 0
              ? [{ quantity: toNumber(state.shortQty), uom_code: state.shortUom }]
              : [],
          damage_description:
    ...
    onSuccess: async (deliveryId) => {
      await queryClient.invalidateQueries({ queryKey: ["goods-receiving-dashboard-deliveries"] });
    ...
    setLineStates((current) =>
      current.map((state) => {
        ...
        const quantityFor = (uom: string) => {
          const factor = factorFor(line.product_id, uom, productUnits, docUom, docFactor);
          if (!factor) return "0";
        ...
        if (status === "Damaged") {
          return {
            ...state,
            status,
            damagedQty: remainingBase > 0 ? quantityFor(state.damagedUom) : state.damagedQty,
          };
        }
        if (status === "Rejected") {
          return {
            ...state,
            status,
            rejectedQty: remainingBase > 0 ? quantityFor(state.rejectedUom) : state.rejectedQty,
          };
        }
        if (status === "Short") {
          return {
            ...state,
            status,
            shortQty: remainingBase > 0 ? quantityFor(state.shortUom) : state.shortQty,
          };
        }
        ...
        {state[key].map((file, index) => {
          ...
          return (
            <div key={`${file.name}-${file.lastModified}-${index}`} className="relative">
              ...
              <button
                type="button"
                onClick={() => removeFile(state.purchaseOrderLineId, key, index)}
  ...
  const detailRows = (line: PurchaseOrderLine, state: LineState) => {
    ...
    const showDamaged = toNumber(state.damagedQty) > 0 || state.status === "Damaged";
    const showRejected = toNumber(state.rejectedQty) > 0 || state.status === "Rejected";
    const showShort = toNumber(state.shortQty) > 0 || state.status === "Short";
    const hasIssue = showDamaged || showRejected || showShort;
    const showAccepted = hasIssue && math.acceptedBase > 0;
          ...
          <div className="space-y-3 p-3 sm:p-4">
            {showAccepted && (
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="font-semibold text-emerald-700">● Received (Good)</p>
                  <p className="text-sm text-emerald-700">
                    Auto balance: <strong>{formatQty(math.acceptedDocQty)} {state.receivedUom}</strong>
                  </p>
                </div>
              </div>
            )}
                ...
                <div className="grid gap-3 lg:grid-cols-[150px_150px_minmax(260px,1fr)_minmax(280px,1fr)]">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        updateLine(state.purchaseOrderLineId, { damagedQty: event.target.value })
                      }
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        updateLine(state.purchaseOrderLineId, { damagedUom: value })
                      }
                    >
                      <SelectTrigger className={INPUT_CLASS}><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {uoms.map((uom) => <SelectItem key={uom} value={uom}>{uom}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Label className="text-amber-800">Damage Description *</Label>
                    <Input
                  ...
                  {issuePhotoStrip(state, "damagePhotos", "Damage Photos *", "#B45309")}
                </div>
              </div>
            )}
                ...
                <div className="grid gap-3 lg:grid-cols-[150px_150px_minmax(260px,1fr)_minmax(280px,1fr)]">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        updateLine(state.purchaseOrderLineId, { rejectedQty: event.target.value })
                      }
                      className={INPUT_CLASS}
                    />
                  </div>
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        updateLine(state.purchaseOrderLineId, { rejectedUom: value })
                      }
                    >
                      <SelectTrigger className={INPUT_CLASS}><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  {issuePhotoStrip(state, "rejectedPhotos", "Rejected Goods Photos *", "#B91C1C")}
                </div>
              </div>
            )}
            ...
            {showShort && (
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-3">
                <div className="grid gap-3 lg:grid-cols-[150px_150px_minmax(260px,1fr)_minmax(280px,1fr)]">
                  <div>
                    <Label className="text-blue-800">Short / Missing Quantity *</Label>
                    <Input
                      type="number"
                      min="0"
                      value={state.shortQty}
                      onChange={(event) =>
                        updateLine(state.purchaseOrderLineId, { shortQty: event.target.value })
                      }
                      className={INPUT_CLASS}
                    />
                  </div>
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        updateLine(state.purchaseOrderLineId, { shortUom: value })
                      }
                    >
                      <SelectTrigger className={INPUT_CLASS}><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        updateLine(state.purchaseOrderLineId, { shortReason: event.target.value })
                      }
                      className={INPUT_CLASS}
                    />
                  </div>
                  {issuePhotoStrip(state, "shortPhotos", "Short / Missing Photos", "#1D4ED8")}
                </div>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <span>
                Total Accounted (Base UOM): <strong>{formatQty(math.accountedBase)}</strong>
              </span>
              <span
                className={
                  math.balanced
    ...
    <div className="space-y-5 p-4 sm:p-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h1 className="text-2xl font-bold">Goods Receiving — Delivery Bill</h1>
            <p className="text-sm text-slate-500">Receive one supplier Delivery Bill at a time.</p>
          </div>
          <div className="flex gap-5 text-xs">
            <span className="font-bold text-[#9E4B4B]">1 Delivery Bill</span>
            <span className={poMatched ? "font-semibold" : "text-slate-400"}>2 Items Inspection</span>
          ...
          <div>
            ...
            <div className="mt-1 flex gap-2">
              <Input
                ...
                className={INPUT_CLASS}
              />
              <Button variant="outline" size="icon" onClick={() => billRef.current?.click()}>
          ...
          <div className="hidden lg:block">
            {billPhoto ? (
              <button
                type="button"
                onClick={() =>
                  setPreview({ url: filePreview(billPhoto), title: "Delivery Bill Photo" })
                }
                className="relative"
              >
                <img
        ...
        <div className="mt-5 rounded-xl border bg-[#F7F9FB] p-4">
              ...
              <SelectContent>
                {purchaseOrders.map((po) => (
                  <SelectItem key={po.purchase_order_id} value={po.purchase_order_id}>
                    {po.purchase_order_no} — {po.suppliers?.supplier_name} — {po.project_sites?.site_name}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {selectedPO && (
            <div className="mt-4 grid gap-3 rounded-xl border bg-white p-3 md:grid-cols-5">
              <div><p className="text-xs text-slate-500">PO No.</p><strong>{selectedPO.purchase_order_no}</strong></div>
              <div><p className="text-xs text-slate-500">Supplier</p><strong>{selectedPO.suppliers?.supplier_name}</strong></div>
              <div><p className="text-xs text-slate-500">Project</p><strong>{selectedPO.projects?.project_name}</strong></div>
              <div><p className="text-xs text-slate-500">Site</p><strong>{selectedPO.project_sites?.site_name}</strong></div>
              <div><p className="text-xs text-slate-500">Expected</p><strong>{selectedPO.expected_delivery_date ?? "-"}</strong></div>
            </div>
          )}
          ...
          {selectedPO && poLines.length > 0 && (
            <div className="mt-4 overflow-hidden rounded-xl border bg-white">
              <div className="border-b bg-[#F7F9FB] px-3 py-2">
                <p className="text-sm font-semibold">Purchase Order Items — Check against Delivery Bill</p>
                <p className="text-xs text-slate-500">Read only. Confirm the product, UOM and PO quantity against the supplier Delivery Bill before enabling inspection.</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[720px] text-sm">
                  ...
                  <tbody>
                    {poLines.map((line) => (
                      <tr key={`verify-${line.purchase_order_line_id}`} className="border-t">
                        <td className="p-2 text-center">{line.line_no}</td>
                        <td className="p-2">
                          <strong>{line.products?.product_name ?? line.description ?? "-"}</strong>
                          <p className="text-xs text-slate-500">{line.products?.product_code ?? ""}</p>
                        </td>
                        <td className="p-2 text-center">{line.purchase_uom_code || line.unit_of_measure}</td>
                        <td className="p-2 text-right font-semibold">{formatQty(line.quantity)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selectedPO && poMatched && (
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b p-5 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-bold">Items on this Delivery Bill ({poLines.length})</h2>
              <p className="text-xs text-slate-500">
                Select items to inspect. Unchecked items are not included in this receiving and are not Rejected.
              </p>
            </div>
            ...
            <table className="w-full min-w-[1080px] text-sm">
              <thead className="bg-[#F7F9FB]">
                <tr>
                  ...
                  <th className="p-3">Accepted / Good</th>
              ...
              <tbody>
                {poLines.map((line) => {
                  const state = lineStates.find(
                    (candidate) =>
                      candidate.purchaseOrderLineId === line.purchase_order_line_id,
                  );
                  if (!state) return null;
                  const math = lineMath(line, state);
                  return (
                    <>
                      <tr key={`${line.purchase_order_line_id}-main`} className="border-t">
                        ...
                        <td className="p-3 text-center">{formatQty(line.quantity)}</td>
                        <td className="p-3">
                          <div className="mx-auto flex max-w-48 items-center gap-2">
                            <Input
                              ...
                              value={state.billQty}
                              onChange={(event) =>
                                updateLine(state.purchaseOrderLineId, { billQty: event.target.value })
                              }
                              className={INPUT_CLASS}
                              disabled={!state.selected}
                            />
                            <span className="min-w-12 text-xs text-slate-500">{state.billUom}</span>
                          </div>
                        </td>
                        <td className="p-3">
                          <div className="mx-auto flex max-w-48 items-center gap-2">
                            <Input
                              ...
                              onChange={(event) =>
                                updateLine(state.purchaseOrderLineId, { receivedQty: event.target.value })
                              }
                              className={INPUT_CLASS}
                              disabled={!state.selected}
                            />
                            <span className="min-w-12 text-xs text-slate-500">{state.receivedUom}</span>
                          </div>
                        </td>
                        <td className="p-3">
                          <Select
                            value={state.status}
                            onValueChange={(value) =>
                              changeStatus(state.purchaseOrderLineId, value as RowStatus)
                            }
                            disabled={!state.selected}
                          >
                            <SelectTrigger
                            ...
                            <SelectContent>
                              <SelectItem value="Received">Received (All)</SelectItem>
                              <SelectItem value="Damaged">Damaged</SelectItem>
                              <SelectItem value="Rejected">Rejected</SelectItem>
                              <SelectItem value="Short">Short / Missing</SelectItem>
                            </SelectContent>
                          </Select>
                          {state.selected && !math.balanced && math.remainingBase > 0 && (
          ...
          <div className="grid gap-3 border-t bg-amber-50 p-4 sm:grid-cols-4">
            <div className="text-emerald-700">
              Accepted <strong className="float-right">{formatQty(summary.accepted)}</strong>
            </div>
            <div className="text-amber-700">
              Damaged <strong className="float-right">{formatQty(summary.damaged)}</strong>
            </div>
            <div className="text-red-700">
              Rejected <strong className="float-right">{formatQty(summary.rejected)}</strong>
            </div>
            <div className="text-blue-700">
              Short / Missing <strong className="float-right">{formatQty(summary.short)}</strong>
            </div>
          </div>
        </section>
      )}
          ...
          <div className="grid gap-4 lg:grid-cols-2">
              ...
              <Select value={stockLocationId} onValueChange={setStockLocationId}>
                ...
                <SelectContent>
                  {stockLocations.map((location) => (
                    <SelectItem key={location.stock_location_id} value={location.stock_location_id}>
            ...
            <div className="rounded-xl border bg-[#F7F9FB] p-3">
              ...
              <p className="mt-1 flex items-center gap-1 text-xs text-slate-500">
                <Lock className="h-3 w-3" />Login user
              </p>
            </div>
          </div>
      ...
      {selectedPO && poMatched && (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-xs text-slate-500">
            On mobile, photos stay below the item inspection. Tap any photo to enlarge.
          </p>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border bg-white p-3">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <Button variant="outline" className="h-28 w-36" onClick={() => billRef.current?.click()}>
            ...
            <div className="rounded-xl border bg-white p-3">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {deliveredGoodsPhotos.map((file, index) => {
                  ...
                  return (
                    <div key={`${file.name}-${file.lastModified}-${index}`} className="relative">
                      <button
                        type="button"
                        onClick={() => setPreview({ url, title: "Delivered Goods Photo" })}
                      >
                        <img
                          src={url}
                          alt="Delivered Goods"
                ...
                <Button variant="outline" className="h-28 w-36" onClick={() => goodsRef.current?.click()}>
      ...
      {selectedPO && poMatched && (
        <div className="flex justify-end gap-2">
          ...
          <Button
            ...
            onClick={openReview}
          >
            <ChevronRight className="mr-2 h-4 w-4" />Review & Confirm
          </Button>
        </div>
      )}
      ...
      {preview && (
        ...
        >
          ...
          <img src={preview.url} alt={preview.title} className="max-h-[92vh] max-w-[96vw] object-contain" />
        </div>
      )}
    </div>
  );
  ...
  const renderReview = () => (
    <div className="space-y-5 p-4 sm:p-6">
      <Button variant="ghost" onClick={() => setStage("entry")}>
        <ArrowLeft className="mr-2 h-4 w-4" />Back to Items Inspection
      </Button>

      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h1 className="text-2xl font-bold">Review & Confirm Goods Receiving</h1>
            <p className="text-sm text-slate-500">
              Check the Delivery Bill, linked PO, quantities, issues and evidence before posting stock.
            </p>
          </div>
          <div className="flex gap-5 text-xs">
        ...
        <div className="mt-5 grid gap-3 rounded-xl border bg-[#F7F9FB] p-4 md:grid-cols-3 lg:grid-cols-6">
          <div><p className="text-xs text-slate-500">Delivery Bill No.</p><strong>{deliveryBillNo}</strong></div>
          <div><p className="text-xs text-slate-500">PO No.</p><strong>{selectedPO?.purchase_order_no}</strong></div>
          <div><p className="text-xs text-slate-500">Supplier</p><strong>{selectedPO?.suppliers?.supplier_name}</strong></div>
          <div><p className="text-xs text-slate-500">Site</p><strong>{selectedPO?.project_sites?.site_name}</strong></div>
          <div><p className="text-xs text-slate-500">Stock Location</p><strong>{stockLocations.find((location) => location.stock_location_id === stockLocationId)?.location_name ?? "-"}</strong></div>
          <div><p className="text-xs text-slate-500">Received By</p><strong>{employeeName(currentEmployee)}</strong></div>
        </div>
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="font-bold">Receiving Summary (Base UOM)</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-700">
            <p className="text-xs">Accepted</p><strong className="text-xl">{formatQty(summary.accepted)}</strong>
          </div>
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-700">
            <p className="text-xs">Damaged</p><strong className="text-xl">{formatQty(summary.damaged)}</strong>
          </div>
          <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-700">
            <p className="text-xs">Rejected</p><strong className="text-xl">{formatQty(summary.rejected)}</strong>
          </div>
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-blue-700">
            <p className="text-xs">Short / Missing</p><strong className="text-xl">{formatQty(summary.short)}</strong>
          </div>
        </div>
      </section>
          ...
          <table className="w-full min-w-[760px] text-sm">
            <thead className="bg-[#F7F9FB]">
              <tr>
                ...
                <th className="p-3">Accepted / Good</th>
                <th className="p-3">Status</th>
                <th className="p-3">Accepted (Base)</th>
            ...
            <tbody>
              {lineStates.filter((state) => state.selected).map((state) => {
                const line = poLines.find(
                  (candidate) => candidate.purchase_order_line_id === state.purchaseOrderLineId,
                )!;
                const math = lineMath(line, state);
                return (
                  <tr key={state.purchaseOrderLineId} className="border-t">
                    <td className="p-3 text-center">{line.line_no}</td>
                    <td className="p-3"><strong>{line.products?.product_name ?? line.description}</strong></td>
                    <td className="p-3 text-center">{formatQty(state.receivedQty)} {state.receivedUom}</td>
                    <td className="p-3 text-center"><span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(state.status)}`}>{statusLabel(state.status)}</span></td>
                    <td className="p-3 text-center">{formatQty(math.acceptedBase)}</td>
                    <td className="p-3 text-center">{formatQty(math.damagedBase)}</td>
                    <td className="p-3 text-center">{formatQty(math.rejectedBase)}</td>
                    <td className="p-3 text-center">{formatQty(math.shortBase)}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={() => setStage("entry")}>Back</Button>
        <Button
          className="text-white"
          style={{ backgroundColor: REDS_ACTION }}
